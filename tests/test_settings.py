...
def test_authenticated_channel_is_fail_closed_and_clamped():
    ...
    assert defaults.authenticated_private_sender_allowlist == frozenset()
    assert defaults.authenticated_secret_file.startswith("/AstrBot/data/secrets/")

    configured = Settings.from_mapping(
        {
            "authenticated_private_sender_allowlist": [" 10001 ", "", 10002],
    ...
    assert configured.authenticated_private_sender_allowlist == frozenset(
        {"10001", "10002"}
    )
...
def test_authenticated_subject_requires_enabled_explicit_private_sender():
    settings = Settings(
        ...
        authenticated_private_sender_allowlist=frozenset({"10001"}),
    )

    assert settings.authenticated_subject_for("10001", "") == "10001"
