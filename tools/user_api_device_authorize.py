    ...
    *,
    ...
    try:
        ...
        if exc.code == 403 and marker == "challenge":
            raise AuthorizationError("Cloudflare challenge blocked device authorization") from exc
...
def _load_state(session_dir: Path) -> tuple[dict[str, object], Path]:
    state_file = session_dir / "state.json"
    ...
    if (
        ...
        or state.get("flow") != "device"
        or state.get("site") != SITE
        or not isinstance(state.get("device_code"), str)
